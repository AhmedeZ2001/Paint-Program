...
void FillRectangle(HDC hdc,int x1, int y1,int x2,int y2,int bit,COLORREF color)
{
    Vector v[4];
    LineDDA(hdc, x1, y1, x2, y1, color);
    ...
    LineDDA(hdc, x2, y1, x2, y2, color);
    // if bit =0 draw bezier curves horizontal
    if(bit==0)
    {
        if(y1>y2)
            swap(x1,y1,x2,y2);
        ...
        while(ys<ye)
        {
            ...
            DrawBezierCurve(hdc,v[0],v[1],v[2],v[3],RGB(255,0,0));
            ys++;
        }
    }

    // else draw hermite curves vertical
    else{
        if(x1>x2)
            swap(x1,y1,x2,y2);
        ...
        while(xs<xe)
        {
            ...
            drawHermite(hdc,v[0],T1,v[3],T2,RGB(255,0,0));
            xs++;
        }
    }
}
void fillSquare(HDC hdc,int x1, int y1, double sidelength , int bit ,COLORREF color)
{
    double halfside = sidelength;
    Vector v[4];
    LineDDA(hdc,x1 + halfside, y1 - halfside,x1 + halfside, y1 + halfside,color);
    ...
    if(bit==0)
    {
        int ys=y1-halfside+1,ye=y1+halfside;
        while(ys<ye)
        {
            v[0][0] = x1-halfside+1;
            v[0][1] = ys;
    ...
    // else draw hermite curves vertical
        ...
        int xs=x1-halfside+1,xe=x1+halfside;
        while(xs<xe)
        {
            v[0][0] = xs;
            v[0][1] = y1-halfside+1;
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_RBUTTONUP:
        ...
        if(choose==4){
            ...
            fillSquare(hdc,x,y,sideLength,1,color);
        }
        else if(choose==42){
            fillSquare(hdc,x,y,sideLength,0,color);
        }
        else if(choose==43){
            if(p1==0){
            x2=LOWORD(lParam);
            ...
            FillRectangle(hdc,x,y,x2,y2,1,RGB(0,0,0));
            p1=0;
            }
        }
        else if(choose==44){
            ...
            FillRectangle(hdc,x,y,x2,y2,0,RGB(0,0,0));
            p1=0;
            }
        }
    case WM_CREATE:
	{
        ...
        AppendMenu(fillcurvesquare, MF_STRING,42 ,"Bezier (Horizontal)");

        AppendMenu(fillcurve, MF_POPUP, (UINT_PTR)fillcurverectangle, "Rectangle");
        AppendMenu(fillcurverectangle, MF_STRING,43 ,"Hermite (Vertical)");
